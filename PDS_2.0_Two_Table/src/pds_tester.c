...
// PDS function wrappers mapped to your correct parameter order
...
int pds_open(char* tname1, int rec_size1, char* tname2, int rec_size2) { 
    return db_open(tname1, tname2, rec_size1, rec_size2); // Fixed order here too!
}

int add_record(char* tname, int key, void* record) {
    return table_store(tname, key, record); // tname comes first!
}

int search_record(char* tname, int key, void* record) {
    return table_get(tname, key, record); // tname comes first!
}

int update_record(char* tname, int key, void* record) {
    return table_update(tname, key, record); // tname comes first!
}

int delete_record(char* tname, int key) {
    return table_delete(tname, key); // tname comes first!
}

int undelete_record(char* tname, int key) {
    return table_undelete(tname, key); // tname comes first to match your pds2.h!
}
